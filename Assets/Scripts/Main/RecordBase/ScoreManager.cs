...
public class ScoreManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (MainGame.IndGameLevel != null)
        {
            ...
            DataTable table = DataBase.GetTable($"SELECT * FROM USER_SOLUTIONS WHERE LEVEL_ID = {level_id};");
        }
    }
    ...
    */
    public void InputStatesAndSymbols(List<string> states, List<string[][]> symbols)
    {
        ...
        if (DataBase.GetTable($"SELECT * FROM USER_SOLUTIONS WHERE LEVEL_ID = {level_id};").Rows.Count == 0)
        {
            DataBase.ExecuteQueryWithoutAnswer($"INSERT INTO USER_SOLUTIONS(LEVEL_ID, USER_STATES, USER_SYMBOLS) VALUES('{level_id}', '{str_states}', '{str_symbols}');");
        ...
        else
        {
            ...
            DataBase.ExecuteQueryWithoutAnswer($"UPDATE USER_SOLUTIONS SET USER_STATES = '{str_states}', USER_SYMBOLS ='{str_symbols}' WHERE LEVEL_ID = '{level_id}'");
